...
def get_biased_samples(img, sample_size=5000, edge_weight=10.0):
    ...
    # 1. Edge Detection (Catches borders)
    edges = analysis_img.convert('L').filter(ImageFilter.FIND_EDGES)
    
    # 2. Luminance map (To catch flat, bright torch centers and deep shadows)
    ...
    # Boost Extremes: Distance from mid-gray (128). 
    # Pure black and pure white get the maximum boost.
    extremes = np.abs(l_vals - 128.0) / 128.0 
    weights += extremes * (edge_weight * 1.5) # Give extremes slightly more power than edges
...
# =====================================================================
# CORE QUANTIZATION PIPELINE WITH L-WEIGHT
# =====================================================================
def extract_palette(img, num_colors, prev_palette_lab=None, edge_weight=10.0, samples=5000, l_weight=2.0):
    sampled_rgb = get_biased_samples(img, samples, edge_weight)
    sampled_lab = rgb_to_lab(sampled_rgb)
    
    # --- AXIS STRETCHING (The trick to force luminance priority) ---
    sampled_lab[:, 0] *= l_weight
    
    if prev_palette_lab is not None and len(prev_palette_lab) == num_colors:
        seed_lab = prev_palette_lab.copy()
        seed_lab[:, 0] *= l_weight # Stretch the seed to match our warped space
    ...
    # --- AXIS COMPRESSION (Un-warp back to true LAB space) ---
    centroids[:, 0] /= l_weight
...
# BATCH PROCESSOR
# =====================================================================
def process_video_frames(input_dir, output_dir, num_colors, scale_factor, blend_factor, edge_weight, samples, l_weight):
    ...
    print(f"Starting L-Weighted CIELAB Pipeline on {total_files} frames...")
    ...
    for i, img_file in enumerate(files_to_process, 1):
        ...
        try:
            ...
            # Extract target with L-weighting
            target_palette_lab = extract_palette(img, num_colors, prev_palette_lab, edge_weight, samples, l_weight)
...
if __name__ == "__main__":
    ...
    # New L-Weight Parameter
    parser.add_argument("--l-weight", type=float, default=2.5,
                        help="Multiplier for Luminance importance in K-Means (default: 2.5)")
    
    args = parser.parse_args()
    process_video_frames(args.input_dir, args.output_dir, args.colors, args.scale, args.blend, args.edge_weight, args.samples, args.l_weight)