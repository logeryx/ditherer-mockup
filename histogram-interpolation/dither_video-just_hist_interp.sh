...
# Pointing to the new histogram script
PYTHON_SCRIPT="scripts/interpolate-histograms.py"
# ==============================================================================
...
# 2. Extract Base Filename (e.g. "my_gameplay.mp4" -> "my_gameplay")
...
echo "---------------------------------------------------------"
echo " Started Fluid Histogram Pipeline for: $BASENAME"
echo " Working Directory: $WORK_DIR"
echo " Settings: $COLORS Colors, ${SCALE}x Scale, $BLEND Blend, $TRANSFER Transfer"
echo "---------------------------------------------------------"

# 4. Detect Framerate
echo "[1/4] Detecting framerate..."
# ffprobe extracts the exact framerate (often a fraction like 60000/1001)
...
# -q:v 2 ensures high-quality JPEG extraction to avoid compression artifacts before dithering
...
.venv/bin/python "$PYTHON_SCRIPT" "$DIR_IN" "$DIR_OUT" -c $COLORS -s $SCALE --blend $BLEND -b $BIN_SIZE --transfer $TRANSFER
...
# Appended "_t${TRANSFER}" to the output name so you know it used the histogram logic
OUTPUT_VIDEO="${BASENAME_NO_EXT}_hist_c${COLORS}_s${SCALE}_b${BLEND}_t${TRANSFER}.mp4"

# Note: The python script outputs as <original_name>-dither.png
...
echo " -> $OUTPUT_VIDEO"
echo ""
echo " Note: Temporary frames are taking up space in '$WORK_DIR'."
echo "       You can safely delete that folder if you are happy with the video."
echo "---------------------------------------------------------"