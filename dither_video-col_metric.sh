...
echo "---------------------------------------------------------"
echo " Started Adaptive Histogram Pipeline for: $BASENAME"
echo " Working Directory: $WORK_DIR"
echo " Target Colors: $COLORS (Max Error: $MAX_DIST)"
echo " Settings: ${SCALE}x Scale, $BLEND Blend, $TRANSFER Transfer"
echo "---------------------------------------------------------"
...
.venv/bin/python "$PYTHON_SCRIPT" "$DIR_IN" "$DIR_OUT" -c $COLORS -d $MAX_DIST -s $SCALE --blend $BLEND -b $BIN_SIZE --transfer $TRANSFER
...
OUTPUT_VIDEO="${BASENAME_NO_EXT}_hist_c${COLORS}_dist${MAX_DIST}_s${SCALE}.mp4"
